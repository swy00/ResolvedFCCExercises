...
Add a constructor to this component and initialize it with a state that has two properties: input, that's set to an empty string, and messages, that's set to an empty array.
...
Also, the input element should render the value of input that's in the component's state. The button element should trigger a submitMessage() method when it's clicked.
    ...
    render() {
      return (
        <div>
          ...
          {/*<div>{this.state.input}</div> Test de input*/}
...
The initial state should equal an empty array. This reducer should add a message to the array of messages held in state, or return the current state. 
...
const messageReducer2 = (previousState=[],action)=>{
...
const store2 = Redux.createStore(messageReducer);
...
The object should have a property submitNewMessage set to the dispatch function, which takes a parameter for the new message to add when it dispatches addMessage().
*/
...
Now that you've written both the mapStateToProps() and the mapDispatchToProps() functions, you can use them to map state and 
dispatch to the props of one of your React components. The connect method from React Redux can handle this task. 
...
const addMessage5 = (message) => {
  ...
  // Change code below this line
  const ConnectedComponent = connect(mapStateToProps4, mapDispatchToProps4)(Presentational)
...
const messageReducer11 = (state = [], action) => {
  switch (action.type) {
    case ADD:
      return [
        ...state,
...
const store11 = Redux.createStore(messageReducer11);
...
const connect1 = ReactRedux.connect;

// Define the Container component here:
const Container = connect(mapStateToProps11, mapDispatchToProps11)(Presentational);
...
These messages will be managed by Redux. Next, modify the submitMessage() method so that it 
...
const messageReducer21 = (state = [], action) => {
  switch (action.type) {
    case ADD:
      return [
        ...state,
...
const store21 = Redux.createStore(messageReducer21);

// React:
const Provider21 = ReactRedux.Provider;
const connect21 = ReactRedux.connect;
...
const mapStateToProps21 = (state) => {
  return {messages: state}
};

const mapDispatchToProps21 = (dispatch) => {
  return {
    submitNewMessage: (message) => {
      dispatch(addMessage(message))
    }
...
const Container21 = connect(mapStateToProps21, mapDispatchToProps21)(Presentational);
...
There's one last item worth pointing out before you move on. 
Typically, you won't write React apps in a code editor like this. 
This challenge gives you a glimpse of what the syntax looks like if you're working with npm and a file system on your own machine. 
...
This can get complicated quickly. If you are interested in experimenting on your own machine, the Create React App comes configured and ready to go.
